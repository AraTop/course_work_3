...
class AuthService:
   ...
   def create_tokens(self, email, password, password_is ):
   ...
   def refresh_token(self, refresh_token):
      """получение refresh токена и return tokens"""

      data = jwt.decode(jwt=refresh_token, key=JWT_SECRET ,algorithms=[JWT_ALGORITHM])
      id = data.get("id")
      user = self.user_service.get_one(id)
      return self.create_tokens(user.email ,None, password_is=True)